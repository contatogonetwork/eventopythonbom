...
class TimelineWindow(QWidget, EventAwareWindow):
    ...
    def edit_task(self):
        ...
        # Verificar se o item existe antes de tentar acessar o text()
        item = self.tasks_table.item(selected_row, 6)
        if item is None:
            QMessageBox.warning(self, "Erro", "Não foi possível acessar o ID da tarefa selecionada.")
            return
