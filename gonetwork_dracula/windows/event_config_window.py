...
class EventConfigWindow(QWidget, EventAwareWindow):
    ...
    def on_event_changed(self, event_id, event_name):
        ...
        try:
            # Verificar se o widget ainda existe antes de tentar modificá-lo
            ...
            # Captura erros quando widgets já foram destruídos
            pass
    ...
    def save_event(self):
        ...
        # Commit e obter dados necessários
        session.commit()
        event_id = event.id
        event_name = event.name
        session.close()
        
        # Primeiro remover essa janela como observadora para evitar notificação após fechar
        self.event_context.remove_observer(self)
        
        # Fechar a janela antes de atualizar o contexto
        self.close()
        
        # Agora atualizar o contexto após a janela ser fechada
